...
const componentMatcher = new MatchDecorator({
  ...
  decorate: (add, from, to, match) => {
    // match[0] is e.g. "<InfoBox" or "</InfoBox"
    // Bracket part: "<" or "</"
...
export const mdxComponentHighlight = ViewPlugin.fromClass(
  class {
    ...
    constructor(view: EditorView) {
      this.decorations = componentMatcher.createDeco(view)
    }
    update(update: ViewUpdate) {
      this.decorations = componentMatcher.updateDeco(update, this.decorations)
    }
  },
